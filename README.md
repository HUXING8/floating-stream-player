...
- **鼠标移动隐藏**：鼠标活动时自动淡出、静止时显示，三种模式可选。
- **最小化角标**：一键缩成左上角小图标，视频后台继续播，点图标展开。
- **登录支持**：独立登录窗口与播放器共享会话，登录一次后看需登录的内容。
- **全局快捷键**：隐藏 / 显示、切换穿透、静音，均可自定义。
- **主题**：明亮 / 夜间 / 跟随系统。
- **原生控件**：进度条、播放暂停、音量；自动跳过可跳过的广告。
...
5. **最小化**：点「—」缩成左上角图标，点图标展开。
6. **设置**（⚙）：主题、不透明度、音量、鼠标移动隐藏、快捷键。
...
npm run dist         # electron-builder 打包 Windows 安装包
```

技术栈：Electron · TypeScript · electron-vite · hls.js · mpegts.js · electron-store。

```
src/
  shared/      配置 schema 与 IPC 契约（主/渲染共用，唯一真源）
  main/        主进程：窗口、配置存储、全局快捷键、光标监听、登录窗口
  preload/     contextBridge 暴露 API
  renderer/
    player/    悬浮窗：直链/页面播放、视频提取、可见性控制、工具栏
    settings/  设置窗口
```
